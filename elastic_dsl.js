...
function leftRight(operator,data){
        if(data.left.value == 'time') return;
        ...
        switch( operator ){
          case "=":
                bool.must.push( { match: { [data.left.value]: data.right.value }} );
                break;
          case "NOT":
                bool.must_not.push( { match: { [data.left.value]: data.right.value }} );
                break;
          case ">":
                range[data.left.value] = {};
                range[data.left.value].gte = data.right.value;
                break;
          case "<":
                range[data.left.value] = {};
                range[data.left.value].lte = data.right.value;
                break;
          case "BETWEEN":
                range[data.left.value] = {};
                range[data.left.value].gte = data.right.value[0];
                range[data.left.value].lte = data.right.value[1];
                break;
        }
  }
  ...
  parse(sql) {
    ...
    walk(result, node => {
      if(node.type == 'operator') {
           ...
           leftRight(node.operator,node);

           if(node.left && node.right){
                // Resolve Time function
                if(node.left.value == "time"){
                   if(!range['@timestamp']) range['@timestamp'] = {};
                   ...
                   if(node.right.left && node.right.right) {
                           ...
                           if(node.right.right.range && node.right.operator){
                                ...
                                if (node.right.operator == '-') range['@timestamp'].gte = new Date().getTime() - timestring(timeeq)
                                else if (node.right.operator == '+') range['@timestamp'].gte = new Date().getTime() + timestring(timeeq)
                                range['@timestamp'].lte = new Date().getTime();
                           }
                   }
    ...
    if (Array.isArray(returnColumns)){
        var aggs = {};
        var parent = [];
        returnColumns.forEach(function(key,i){
          parent.push(key.name);
          var jid = parent.join('.')
          index(aggs,jid,{ "terms" : { "field" : key.sourceColumns[0].value || key.value } });
